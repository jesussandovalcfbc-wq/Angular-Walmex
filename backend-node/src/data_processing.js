...
exports.cargarGasolina = cargarGasolina;
exports.cargarNomina = cargarNomina;
...
const GASOLINA_SHARED_LINK = "https://pacificafarms-my.sharepoint.com/:x:/g/personal/anahi_mora_cfbc_co/IQANE_rjNbe-T5n5XZuwt3FwAa9dla1RGnbl1oC9PGuNO-o?e=U5ZEPB";
...
async function cargarGasolina() {
    try {
        const buffer = await (0, sharepoint_1.downloadViaGraphSharedLink)(GASOLINA_SHARED_LINK);
        const wb = XLSX.read(buffer, { type: 'buffer' });
        const sheetName = wb.SheetNames.find(s => s.toLowerCase() === 'base datos') || wb.SheetNames[0];
        if (!sheetName)
            throw new Error('El archivo de gasolina no contiene hojas.');
        ...
        for (let i = 3; i < rows.length; i++) {
            const row = rows[i];
            if (!row || row.length <= Math.max(COL_FECHA, COL_TOTAL, COL_VEH))
                continue;
            const vehiculo = String(row[COL_VEH] || '').trim();
            if (!VEHICULOS_GASOLINA.includes(vehiculo))
                continue;
        ...
        return {
            ...
            gasolina_semanas: Array.from(semanasSet).sort((a, b) => a - b),
...
async function cargarNomina() {
    try {
        const buffer = await (0, sharepoint_1.downloadViaGraphSharedLink)(NOMINA_SHARED_LINK);
        const wb = XLSX.read(buffer, { type: 'buffer' });
        const resultado = {};
        const semanasSet = new Set();
        ...
        return {
            ...
            nomina_semanas: Array.from(semanasSet).sort((a, b) => a - b)
        };
    }
    catch (e) {
...
async function cargarDatos(cacheKey = "") {
    if (!fs.existsSync(CACHE_DIR))
        fs.mkdirSync(CACHE_DIR, { recursive: true });
    let sourceLastModified = "";
    const withSourceMeta = (data) => {
        if (sourceLastModified)
            data._source_last_modified = sourceLastModified;
        return data;
    };
    ...
    if (!cacheKey) {
        try {
            const meta = await (0, sharepoint_1.getSharepointFileMeta)();
    ...
    if (fs.existsSync(CACHE_LATEST)) {
        try {
            ...
            if (d.resumen_diario && (!cacheKey || d._source_cache_key === cacheKey))
                return withSourceMeta(d);
        }
        catch (e) { }
    }
    ...
    const { localPath, cacheKey: downloadedCacheKey } = await (0, sharepoint_1.downloadExcelSharepoint)();
    ...
    const ctdNombres = { 'Sab': 'Cnt Sab', 'Dom': 'Ctd Dom', 'Lun': 'Ctd Lun', 'Mar': 'Ctd Mar', 'Mie': 'Ctd Mie', 'Jue': 'Ctd Jue', 'Vie': 'Ctd Vie' };
    const idxCtd = {}, idxVtas = {};
    for (const d of dias) {
    ...
    for (let i = 1; i < rows.length; i++) {
        const row = rows[i];
        if (!row || !row[idxProd] || !row[idxTienda] || !row[idxSemana])
            continue;
    ...
    const semanas = Array.from(new Set(records.map(r => r.semana))).sort((a, b) => a - b);
    ...
    const byStp = {};
    const fechaPorSemana = {};
    const totalesTienda = {};
    const rawSemana = {};
    const totalesProdTienda = {};
    const resumenDiario = {};
    const weekStartsToSemana = {};
    for (const r of records) {
        ...
        if (!byStp[sem][t])
            byStp[sem][t] = {};
        if (!byStp[sem][t][p])
            byStp[sem][t][p] = { ventas_u: 0, embarque_u: 0, merma_u: 0, venta_cfbc: 0, venta_wmx: 0, retail_vc: 0, inventario: 0 };
        const b = byStp[sem][t][p];
        b.ventas_u += r.ventas_u;
        b.embarque_u += r.embarque_u;
        b.merma_u += r.merma_u;
        b.venta_cfbc += r.venta_cfbc;
        b.venta_wmx += r.venta_wmx;
        b.retail_vc += r.retail_vc;
        b.inventario += r.inventario;
        for (const d of dias) {
        ...
        if (!totalesTienda[t])
            totalesTienda[t] = { embarque_u: 0, venta_cfbc: 0, venta_wmx: 0, merma_u: 0, retail_vc: 0, inventario: 0, ventas_u: 0 };
        const tt = totalesTienda[t];
        tt.embarque_u += r.embarque_u;
        tt.venta_cfbc += r.venta_cfbc;
        tt.venta_wmx += r.venta_wmx;
        tt.merma_u += r.merma_u;
        tt.retail_vc += r.retail_vc;
        tt.inventario += r.inventario;
        tt.ventas_u += r.ventas_u;
        if (!rawSemana[t])
            rawSemana[t] = {};
        if (!rawSemana[t][sem])
            rawSemana[t][sem] = { embarque_u: 0, venta_cfbc: 0, venta_wmx: 0, merma_u: 0, retail_vc: 0, inventario: 0, ventas_u: 0 };
        const rs = rawSemana[t][sem];
        rs.embarque_u += r.embarque_u;
        rs.venta_cfbc += r.venta_cfbc;
        rs.venta_wmx += r.venta_wmx;
        rs.merma_u += r.merma_u;
        rs.retail_vc += r.retail_vc;
        rs.inventario += r.inventario;
        rs.ventas_u += r.ventas_u;
        if (!totalesProdTienda[t])
            totalesProdTienda[t] = {};
        if (!totalesProdTienda[t][p])
            totalesProdTienda[t][p] = { embarque_u: 0, venta_cfbc: 0, venta_wmx: 0, merma_u: 0, retail_vc: 0, inventario: 0, ventas_u: 0 };
        const tp = totalesProdTienda[t][p];
        tp.embarque_u += r.embarque_u;
        tp.venta_cfbc += r.venta_cfbc;
        tp.venta_wmx += r.venta_wmx;
        tp.merma_u += r.merma_u;
        tp.retail_vc += r.retail_vc;
        tp.inventario += r.inventario;
        tp.ventas_u += r.ventas_u;
        for (const d of dias) {
            const kl = d.toLowerCase();
        ...
        if (r.fecha_ymd) {
            if (!resumenDiario[t])
                resumenDiario[t] = {};
            if (!resumenDiario[t][sem])
                resumenDiario[t][sem] = {};
            if (!resumenDiario[t][sem][p])
                resumenDiario[t][sem][p] = {};
            if (!resumenDiario[t][sem][p][r.fecha_ymd])
                resumenDiario[t][sem][p][r.fecha_ymd] = { embarque: 0, ventas: 0, merma: 0, venta_pos: 0, venta_cfbc: 0, field1: 0, inventario: 0 };
            const rd = resumenDiario[t][sem][p][r.fecha_ymd];
            rd.embarque += r.embarque_u;
            rd.ventas += r.ventas_u;
            rd.merma += r.merma_u;
            rd.venta_pos += r.venta_pos;
            rd.venta_cfbc += r.venta_cfbc;
            rd.field1 += r.field1;
    ...
    for (const t of tiendas) {
        ...
        for (const s of semanas) {
            ...
            for (const p of productos) {
                ...
                for (const sem of last12)
                    v12 += (byStp[sem]?.[t]?.[p]?.ventas_u || 0);
                for (const sem of last3) {
                    const b = byStp[sem]?.[t]?.[p] || {};
                    v3 += (b.ventas_u || 0);
                    emb3 += (b.embarque_u || 0);
                    m3 += (b.merma_u || 0);
                    cfbc3 += (b.venta_cfbc || 0);
                    retail3 += (b.retail_vc || 0);
                    wmx3 += (b.venta_wmx || 0);
                }
                const avg = v3 / 3;
    ...
    for (const r of records) {
        ...
        if (ruta && gasto > 0) {
            if (!gastoData[ruta])
                gastoData[ruta] = {};
            if (!gastoData[ruta][r.semana])
                gastoData[ruta][r.semana] = {};
    ...
    for (const sheetName of wb.SheetNames) {
        if (sheetName.trim().toLowerCase() === 'detalle')
            wsDetalle = wb.Sheets[sheetName];
        if (sheetName.trim().toLowerCase() === 'gastos')
            wsGastos = wb.Sheets[sheetName];
        if (sheetName.trim().toLowerCase() === 'reporte-gastosapp')
            wsReporteGastosApp = wb.Sheets[sheetName];
    }
    const detalleInventario = { fechas: [], fechas_por_semana: {}, fecha_to_semana: {}, data: {} };
    const detalleCuartoFrio = { fechas: [], data: {} };
    if (wsDetalle) {
        const detRows = XLSX.utils.sheet_to_json(wsDetalle, { header: 1 });
                ...
                if (td && pd2 && fechaStr) {
                    ...
                    if (dateObj && !fechaToSemana[fechaStr]) {
                        ...
                        if (sem) {
                            ...
                            if (!fechasPorSemana[sem])
                                fechasPorSemana[sem] = [];
                            if (!fechasPorSemana[sem].includes(fechaStr))
                                fechasPorSemana[sem].push(fechaStr);
                        }
                    }
                }
            }
    ...
    const agregarGasto = (ruta, concepto, semKey, monto) => {
        if (!ruta || !concepto || !semKey || monto <= 0)
            return;
        if (!gastosSc.includes(ruta))
            gastosSc.push(ruta);
        if (!gastosTipos.includes(concepto))
            gastosTipos.push(concepto);
        if (!gastosOtros[ruta])
            gastosOtros[ruta] = {};
        if (!gastosOtros[ruta][concepto])
            gastosOtros[ruta][concepto] = {};
        gastosOtros[ruta][concepto][semKey] = (gastosOtros[ruta][concepto][semKey] || 0) + monto;
    };
    if (wsGastos) {
        const gasRows = XLSX.utils.sheet_to_json(wsGastos, { header: 1 });
        const gasHeaders = (gasRows[0] || []).map(h => String(h || '').trim());
        const gastoCols = [];
        for (let i = 2; i < gasHeaders.length; i++) {
            if (gasHeaders[i])
                gastoCols.push({ idx: i, h: gasHeaders[i] });
        }
        const _conceptoGasto = (tipo, detSc) => {
            const tl = tipo.toLowerCase();
            if (tl === 'viaje / caseta')
                return 'VIATICOS_CASETAS';
            if (tl === 'gasolina / diésel')
                return 'GASOLINA';
            if (tl.includes('merma'))
                return 'MERMA_BODEGA';
            if (tl.includes('renta'))
                return 'RENTA_BODEGA';
            if (tl.includes('nomina') || tl.includes('nómina'))
                return 'NOMINA_BODEGA';
            if (tl.includes('empaque'))
                return 'EMPAQUE_INSUMOS';
        ...
        for (let i = 1; i < gasRows.length; i++) {
            ...
            if (dtObj && dtObj >= corteReporteGastosApp)
                continue;
            let semKey = null;
            if (dtObj) {
    ...
    if (wsReporteGastosApp) {
        ...
        if (idxAppTienda >= 0 && idxAppFechaGasto >= 0 && idxAppCategoria >= 0 && idxAppMonto >= 0) {
            for (let i = 1; i < appRows.length; i++) {
                ...
                if (!fechaGasto || fechaGasto < corteReporteGastosApp)
                    continue;
