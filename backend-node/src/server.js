...
async function initData() {
    ...
    try {
        const mainData = await (0, data_processing_1.cargarDatos)();
        const gasData = await (0, data_processing_1.cargarGasolina)();
        const nomData = await (0, data_processing_1.cargarNomina)();
        GLOBAL_DATA = {
            ...mainData,
            ...gasData,
            ...nomData
...
app.get('/api/db-health', async (_req, res) => {
    try {
        await (0, database_1.verifyDatabase)();
...
app.get('/api/facturas-data', async (_req, res) => {
    try {
        const data = await (0, database_1.getFacturasData)();
        res.json(data);
    }
    catch (e) {
...
app.get('/api/devoluciones', async (req, res) => {
    try {
        const data = await (0, database_1.getDevolucionesData)();
        res.json(data);
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
app.patch('/api/devoluciones/verificacion', async (req, res) => {
    try {
        ...
        if (!ids.length)
            return res.status(400).json({ error: 'No se recibieron devoluciones validas.' });
        const rows = await (0, database_1.setDevolucionesVerification)(ids, verified);
...
app.patch('/api/devoluciones/:id/verificacion', async (req, res) => {
    try {
        ...
        if (!Number.isInteger(id) || id <= 0)
            return res.status(400).json({ error: 'Devolucion invalida.' });
        const row = await (0, database_1.setDevolucionVerification)(id, verified);
...
app.post('/api/facturas/:folio/cancelar', async (req, res) => {
    try {
        const folio = String(req.params.folio || '').trim();
        if (!folio)
            return res.status(400).json({ error: 'Folio invalido.' });
        const deleted = await (0, database_1.cancelInvoice)(folio);
...
app.all('/api/db/rest/v1/:table', async (req, res) => {
    try {
        ...
        let data;
        if (req.method === 'GET')
            data = await (0, database_1.restSelect)(table, req.query);
        else if (req.method === 'POST')
            data = await (0, database_1.restInsert)(table, req.query, req.body);
        else if (req.method === 'PATCH')
            data = await (0, database_1.restUpdate)(table, req.query, req.body);
        else if (req.method === 'DELETE')
            data = await (0, database_1.restDelete)(table, req.query);
        else
...
${context}`;
    try {
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
            ...
            continuationCount += 1;
            contents.push({ role: 'model', parts: [{ text: partText }] }, {
                role: 'user',
                parts: [{
...
app.post('/api/upload-excel', async (req, res) => {
    ...
    try {
        ...
        const rowCount = await (0, sharepoint_1.uploadExcelToSharepoint)(fileBuffer);
        (0, data_processing_1.invalidateDashboardCache)();
        const refreshedMainData = await (0, data_processing_1.cargarDatos)();
